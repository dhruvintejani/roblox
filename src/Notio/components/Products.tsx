...
import { aiOptions } from "../../assets/data/Notio/dummydata";
  ...
  return (
    <div className="mx-[70px] overflow-hidden pt-6">
          ...
          <div className="w-[62%] bg-white rounded-3xl shadow-xl border flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              ...
              <div className="flex items-center gap-5">
                <div className="flex items-center gap-2 text-sm cursor-pointer" onClick={() => setPersonalize(!personalize)}>
                  <span>Personalize</span>
                  <div className={`w-10 h-5 rounded-full relative transition ${personalize ? "bg-blue-600" : "bg-gray-300"}`}>
                    <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition ${personalize ? "right-0.5" : "left-0.5"}`} />
                  </div>
                </div>

                <button onClick={() => setOpen(!open)}>
                  <Icon icon="material-symbols:close" className="text-gray-400 hover:text-black" width={24} />
                </button>
              </div>
            </div>
                ...
                >
                  ...
                  {msg.files?.map((file, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-center justify-end gap-3 px-3 py-2 rounded-lg text-sm transition"
                      >
              ...
              <div className="rounded-2xl bg-white p-3 transition border border-blue-200 focus-within:shadow-[0_0_0_4px_rgba(59,130,246,0.15)]">
                ...
                {selectedFiles.length > 0 && (
                  <div className="mb-2 space-y-2">
                    {selectedFiles.map((file, i) => (
                      <div key={i} className="flex items-center justify-between bg-gray-100 rounded-xl px-3 py-2 text-sm">
                        <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <Icon icon="material-symbols:language" width={20} className="cursor-pointer hover:text-blue-500 transition" />
                    <Icon icon="material-symbols:more-horiz" width={20} className="cursor-pointer hover:text-blue-500 transition" onClick={() => setAiModalOpen(true)} />
                  </div>
      ...
      <div className="flex justify-center my-6">
        <button onClick={() => setOpen(!open)} className="px-10 py-3 rounded-full bg-gray-500 text-white hover:bg-black transition">
      ...
      <Modal open={aiModalOpen} onClose={() => setAiModalOpen(false)}>
        <div className="bg-white rounded-2xl shadow-xl p-4 w-[260px]" style={{ position: "absolute", bottom: "65px", left: "51%", transform: "translateX(-50%)" }}>
          {aiOptions.map((ai) => (
            <div
              key={ai.name}
